...
public class WebSocketConstants extends PublicConstants {
     ...
     * websocket 消息队列类型
     */
    public static final String WEBSOCKET_MESSAGE_QUEUE_TYPE = WEBSOCKET_MESSAGE_QUEUE_NAME + "."
            + PublicConstants.MQ_FANOUT;

    /**
     * websocket 消息队列标识
     */
    public static final String WEBSOCKETE_MESSAGW_QUEUE_KEY = WEBSOCKET_MESSAGE_QUEUE_NAME + "."
            + PublicConstants.MQ_KEY;

    /**
     * 存储sessionId和webSocketSession 需要注意的是，webSocketSession没有提供无参构造，不能进行序列化，也就不能通过redis存储
